...
export default function ChatTab({ 
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if ((!inputText.trim() && !questionFile) || submitting || !activeSessionId) return;
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[78vh] rounded-3xl border border-slate-800 bg-slate-950/40 backdrop-blur-md overflow-hidden font-sans relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {!activeSession ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6">
              ...
              <h4 className="text-slate-400 font-bold text-sm">No workspace initialized</h4>
              <p className="text-xs text-slate-600 mt-1 max-w-xs leading-normal">
                Initialize a workspace by clicking <span className="text-indigo-400">"New Inquiry Space"</span> on the sidebar.
              </p>
            </div>
            ...
            <div className="h-full flex flex-col justify-center max-w-2xl mx-auto py-10">
              <div className="text-center mb-8">
                ...
                <h4 className="text-slate-200 font-bold text-base">Direct PDF Prompting Terminal</h4>
                <p className="text-slate-500 text-xs mt-1 max-w-sm mx-auto leading-normal">
                  DocuMind is awaiting your query inside this workspace. The AI will speak based strictly on the context chunks extracted from your active PDFs.
                </p>
              </div>
              ...
              {pdfs.length === 0 ? (
                <div className="p-4 rounded-2xl border border-dashed border-slate-800 bg-slate-900/20 text-center">
                  ...
                  <p className="text-[11px] text-slate-600 mt-1">
                    Upload papers in the Document Manager first for DocuMind to ground its knowledge.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4">
                  <div 
                    onClick={() => selectSuggestion('Summarize the primary objectives, methodologies, and core conclusions across my uploaded documents.')}
                  ...
                  >
                    <span className="text-[11px] font-bold text-indigo-400 uppercase tracking-widest block mb-1 font-mono uppercase group-hover:text-indigo-300">Summarize All</span>
                    <p className="text-xs text-slate-400 leading-normal">Summarize goals and findings in my document store.</p>
                  </div>
                  <div 
                    onClick={() => selectSuggestion('Are there any conflicting findings, legal exceptions, or arguments presented between the papers?')}
                    className="p-3 bg-slate-900/30 hover:bg-slate-900/60 border border-slate-800 rounded-xl text-left cursor-pointer transition select-none group"
                  >
                    <span className="text-[11px] font-bold text-cyan-400 uppercase tracking-widest block mb-1 font-mono uppercase group-hover:text-cyan-300">Contrast Documents</span>
                    <p className="text-xs text-slate-400 leading-normal">Identify differences, debates, or legal overrides.</p>
                  </div>
                </div>
              )}
            </div>
              ...
              {submitting && (
                <div className="flex flex-col items-start">
                  <div className="p-4 bg-indigo-950/20 border border-indigo-900/20 text-indigo-400 rounded-2xl rounded-tl-none flex items-center gap-2.5 shadow-md">
                    ...
                    <span className="text-xs font-semibold animate-pulse font-mono uppercase tracking-widest text-[10px] pl-1">Scanning memory arrays &amp; vector nodes...</span>
                  </div>
                </div>
              )}
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-base font-bold text-white flex items-center gap-2">
                      <Quote className="h-5 w-5 text-indigo-400" /> Source Data Excerpts
                    </h3>
              ...
              <div className="pt-6 border-t border-slate-850 text-center select-none">
                <p className="text-[10px] text-slate-600 leading-normal max-w-xs mx-auto">
                  Only content pieces with critical dot-product counts above threshold are selected to prevent AI slop &amp; hallucinations.
                </p>
              </div>
            </motion.div>
          </motion.div>
