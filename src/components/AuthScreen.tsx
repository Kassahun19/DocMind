...
export default function AuthScreen({ onAuthSuccess, showForm, setShowForm }: AuthScreenProps) {
  ...
  React.useEffect(() => {
    if (showForm) {
      ...
      const mainContainer = document.getElementById('main-landing-container') || document.querySelector('.overflow-y-auto');
      if (mainContainer) {
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {!isLogin && (
                  <div className="flex items-start gap-2.5 pt-1 pb-1 select-none text-left">
                    ...
                    />
                    <label htmlFor="terms-agreement" className="text-[11px] text-slate-400 leading-snug cursor-pointer font-medium">
                      I agree to the <span className="text-indigo-400 hover:text-indigo-300 hover:underline font-semibold transition">Terms of Service</span> and <span className="text-indigo-400 hover:text-indigo-300 hover:underline font-semibold transition">Privacy Policy</span>.
                    </label>
                  </div>
                )}
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleVerifyAdminPrompt} className="p-6 space-y-4">
              <p className="text-xs text-slate-400 leading-relaxed">
                Please enter the administrative email and master password manually to log in as system level controller.
              </p>
